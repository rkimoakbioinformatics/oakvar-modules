...
class Converter(BaseConverter):
    ...
    def setup(self, *args, **kwargs):
        ...
        self.hgvs_systems = ["http://varnomen.hgvs.org",
    ...
    def chrom_finder(self, db, transcript, dict) -> str:
        ...
        if final_str[0] == "E":
            ...
            curs.execute(
                f'SELECT "chromid" from "transcript" WHERE "name" ="{str_tr}"')

            chrom_id = curs.fetchall()[0][0]

            curs.execute(
                f'SELECT "chrom" from "chroms" WHERE "chromid" ={chrom_id}')
    ...
    def check_parsed_code(self, prefix, event, value):
        if self.in_hgvs_comp:
            ...
            if self.chrom_finder(gen_db, value, chrms) is None:
                self.in_hgvs_comp = False
            try:
                self.var_dict['chrom'] = self.chrom_finder(
                    gen_db, value, chrms)
            except:
    ...
    def make_total_lines(self, input_path, num_pool, batch_size):
            ...
            for prefix, event, value in parser:
                ...
                # look at component value
                    ...
                    if self.in_pos_comp is True:
                        if len(component) > 2:
                            self.var_dict["pos"] = int(component[1]['value'])
                            component = []
                            self.in_pos_comp = False
                        else:
        ...
        return temp_lines, immature_exit
