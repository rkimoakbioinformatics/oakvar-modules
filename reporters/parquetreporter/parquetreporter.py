...
class Reporter(BaseReporter):
    def setup(self):
        ...
        self.leading_zeros = len(str(self.results // self.chunk_size))
    ...
    def write_table_row(self, row):
        ...
        if self.rowno % self.chunk_size == 0 or self.rowno == self.results:
            ...
            self.filename = f"{self.filename_prefix}_{self.chunkno:0{self.leading_zeros}}{self.filename_postfix}"
            ...
            result = conn.execute(
                f"COPY {tbl_name} to '{self.filename}' (FORMAT 'PARQUET')"
            )
