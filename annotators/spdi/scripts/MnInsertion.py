...
class MnInsertion(Variant):
    ...
    def _get_left_context(self):
        ...
        while True:
            # Define the range to check, which is the length of alt_base to the left of the current position
            start_pos = left_pos - len(self.alt_base)
            end_pos = left_pos - 1
            bases_at_pos = self._wgs_reader.get_bases(self.chrom, start_pos, end_pos)
            
            # Update the left_context if the bases match the alt_base
            if bases_at_pos == self.alt_base:
                left_context = bases_at_pos + left_context
                left_pos -= len(self.alt_base)
            else:
                break
    ...
    def _get_right_context(self):
        ...
        while True:
            # Define the range to check, which is the length of alt_base to the right of the current position
            start_pos = right_pos
            end_pos = right_pos + len(self.alt_base) - 1
            bases_at_pos = self._wgs_reader.get_bases(self.chrom, start_pos, end_pos)
            print(f"Checking bases at pos {start_pos} to {end_pos}: {bases_at_pos}") # Debugging print
            
            # Update the right_context if the bases match the alt_base
            if bases_at_pos == self.alt_base:
                right_context += bases_at_pos
                right_pos += len(self.alt_base)
            else:
                break
